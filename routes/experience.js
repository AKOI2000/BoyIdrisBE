...
router.get("/experience/:id", async (req, res) => {
  ...
  const result = await pool.query("SELECT * FROM experience WHERE id = $1", [
    id,
  ]);
...
router.post("/experience/create", async (req, res) => {
  ...
  const result = await pool.query(
    "INSERT INTO experience (companyname, role, datestart, dateend) VALUES ($1, $2, $3, $4)",
    [company, role, dateStart, dateEnd]
  );
...
router.patch("/experience/edit/:id", async (req, res) => {
  ...
  const result = await pool.query(
    "UPDATE experience SET companyname = $1, role = $2, datestart = $3, dateend = $4 WHERE id = $5",
    [company, role, dateStart, dateEnd, id]
  );
